        ...
        <div class="row d-flex justify-content-center">
            @if(isset($movies) and count($movies) > 0)
                @foreach($movies as $movie)
                    <div class="col-4 p-1">
                        <div class="card h-100 col-12 div-link">
                            <a class="card-as-link" href="/movies/{{ $movie->id }}">
                                <div class="row no-gutters align-items-center">
                                    <div class="col-md-4">
                                        <div>
                                            <img src="{{ $movie->getMeta('movie_cover') == "N/A" ? "/storage/no_image_found.png" : $movie->getMeta('movie_cover') }}" class="card-img mx-auto d-block movie-cover">
                                        </div>
                                    </div>
                                    <div class="col-md-8">
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="col-8">
                                                    <h4 class="card-title p-1"><strong>{{ str_pad($movie->title, 40) }}</strong></h4>
                                                </div>
                                            </div>
                                            <div class="row">
